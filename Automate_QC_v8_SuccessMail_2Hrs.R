...
tryCatch({#--------- Creating MySQl Connector for Aurora Legacy Tables
user = ''
password = ''
dbname = ""
host = ""
...
#--------- Creating MySQl Connector for Contest service slave DB
user2 = ''
password2 = ''
dbname2 = ""
host2 = ""
port2 =3306
mycontestdb=dbConnect(MySQL(),dbname = dbname2,user =user2,password = password2,
               host = host2,port = port2)

#-----------Creating Redshift looker user Connector
user_red = ''
password_red = ''
dbname_red = ""
host_red = ""
...
redshiftdb=dbConnect(PostgreSQL(),dbname = dbname_red,user =user_red,password = password_red,
                     host = host_red,port = port_red)},error= function(e){print(paste("An error occured while execution for:",table1))
...
contest_tables_Query <- c("select distinct name from d11.cron_timetable 
                          where name like ('Dream11_Contest%')")
all_table_list <- as.data.frame(dbGetQuery(redshiftdb,get_all_tables_Query))
contest_service_table_list <- as.data.frame(dbGetQuery(redshiftdb,contest_tables_Query))

#This Function does standard checks through One Table by taking two List of tables
compare_datasets <- function(x,z,cur_min_7,cur,mydb,redshiftdb) {
  for(i  in 1: nrow(x) ) {
    ...
    table1 <- x[i,1]
    print(table1)
    mysqlquery <- paste("select date(",z,"),count(*),count(distinct id) from dbd11live29june15.",table1," where ",z," between '",cur_min_7,"' and '",cur,"' and date(",z,") <> '0000-00-00' group by 1",sep="")
    ...
    redshiftquery <- c()
    RED_Data <- data.frame()
    redshiftquery <- paste("select date(",z,"),count(*),count(distinct id) from d11.",table1," where ",z," between '",cur_min_7,"' and '",cur,"' group by 1",sep="")
...
compare_datasets_contest_service <- function(x,z,cur_min_7,cur,mydb,redshiftdb) {
  for(i  in 1: nrow(x) ) {
    # -------Querying for Count in MYSQL
    tryCatch({table1 <- c()
    mysqlquery <- c()
    ...
    table1 <- x[i,1]
    print(table1)
    mysqlquery <- paste("select date(",z,"),count(*),count(distinct id) from contestapi.",table1," where ",z," between '",cur_min_7,"' and '",cur,"' and date(",z,") <> '0000-00-00' group by 1",sep="")
    MySQL_Data <- dbGetQuery(mydb,mysqlquery)
    print(paste0("MySQL query executed for ",table1))
    colnames(MySQL_Data) <- c('Date','Count_s_mysql','Count_distinct_mysql')
...
compare_datasets(all_table_list,c("recupdatedat"),current_ts_minus_7,current_ts,mydb,redshiftdb)
compare_datasets_contest_service(contest_service_table_list,c("recupdatedat"),current_ts_minus_7,current_ts,mycontestdb,redshiftdb)
